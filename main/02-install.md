...
버트온은 백업 파일(<point>vzdump-qemu-xxxx.vma.zst</point>)을 사용하여 Proxmox 위 VM으로 빠르게 배포됩니다. 따라서 Proxmox를 미리 구성하는 것을 권장합니다.<br><br>

## 2.1. 백업 파일 배치
다음과 같이 <point>vzdump-qemu-xxxx.vma.zst</point>파일을 Proxmox 스토리지의 <point>백업 디렉토리(Backups)</point>로 이동시킵니다.
* 스토리지 경로 예시 :
